          ...
          rules.forEach(function (rule) {
            if (type === 'all' || !rule.trigger || type === rule.trigger) {
              if (_xeUtils.default.isFunction(rule.validator)) {
                ...
                var numVal = isNumType ? _xeUtils.default.toNumber(cellValue) : _xeUtils.default.getSize(cellValue);

                if (rule.required && (isArrType ? !_xeUtils.default.isArray(cellValue) || !cellValue.length : cellValue === null || cellValue === undefined || cellValue === '') || isNumType && isNaN(cellValue) || !isNaN(rule.min) && numVal < parseFloat(rule.min) || !isNaN(rule.max) && numVal > parseFloat(rule.max) || rule.pattern && !(rule.pattern.test ? rule.pattern : new RegExp(rule.pattern)).test(cellValue)) {
                  _this3.validRuleErr = true;
